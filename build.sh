...
if [ ! -d "$PROJECT_DIR/XPLM-SDK" ]; then
    ...
    echo "Extract it to $PROJECT_DIR/XPLM-SDK/"
    exit 1
fi
...
if [ -f "$XPL_FILE" ]; then
    ...
    echo "1. Create directory: X-Plane/Resources/plugins/ZIBOKeyboardInput/"
    echo "2. Copy mac.xpl to that directory"
    echo "3. Restart X-Plane"
    echo ""
    echo "Plugin Commands (bind in X-Plane settings):"
    echo "- 3370Tech/ZIBO_Keyboard/Toggle_Keyboard_Input_Captain"
    echo "- 3370Tech/ZIBO_Keyboard/Toggle_Keyboard_Input_FO"
else
